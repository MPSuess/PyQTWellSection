...
class CheckableTree(QtWidgets.QTreeWidget):
    leafToggled = QtCore.Signal(str, str, bool)
    parentToggled = QtCore.Signal(str, str, bool)
    ...
    # ---- Unique ID role ----
    _ID_ROLE = QtCore.Qt.UserRole + 500
    _ID_LENGTH = 8
    ...
    def add_root(self, text: str) -> QtWidgets.QTreeWidgetItem:
        ...
        """
        root = self._make_item(text, None, parent=None)
        self.addTopLevelItem(root)
    ...
    def add_parent(self, parent_item, text: str, uid: str, checkable) -> QtWidgets.QTreeWidgetItem:
        """
        ...
        new_item = self._make_item(text, uid, parent=None)
    ...
    def add_leaf(self, parent_item, text: str, uid, checked = False) -> QtWidgets.QTreeWidgetItem:
        """
        ...
        leaf = self._make_item(text, uid, parent=parent_item)
    ...
    def add_checkable_leaf(self, parent_item, text: str, uid:str) -> QtWidgets.QTreeWidgetItem:
        """
        Add an ALWAYS-checkable leaf (checkbox leaf).
        """
        if parent_item is None:
            raise ValueError("add_checkable_leaf requires a parent_item")
        leaf = self._make_item(text, uid, parent_item)
    ...
    def add_noncheckable_leaf(self, parent_item, text: str, uid:str) -> QtWidgets.QTreeWidgetItem:
        """
    ...
    def _add_children(self, parent_item, subtree):
        if isinstance(subtree, dict):
            for k, v in subtree.items():
                child = self._make_item(str(k), None, parent=parent_item)
                self._add_children(child, v)
            return
        if isinstance(subtree, (list, tuple, set)):
            for elem in subtree:
                child = self._make_item(self._label(elem), None, parent=parent_item)
    ...
    def _make_itemo(self, text: str, parent):
    ...
    def _make_item(self, text: str, uid:str , parent: QtWidgets.QTreeWidgetItem | None):
        item = QtWidgets.QTreeWidgetItem([text])
        ...
        self._assign_id(item, uid)  # 🔥 Important
    ...
    def _handle_user_toggle(self, item: QtWidgets.QTreeWidgetItem):
        ...
        if item.childCount() > 0:
            self.parentToggled.emit(self._item_path(item), self.get_item_id(item), checked)
            ...
            for leaf in self._iter_leaves(item):
                self.leafToggled.emit(self._item_path(leaf), self.get_item_id(leaf),checked)
            return

        self.leafToggled.emit(self._item_path(item), self.get_item_id(item),checked)
        try:
            self._updating = True
            self._update_parents(item)
        finally:
 ...
 #       Blocker.unblock()
    ...
    def from_dict(self, data: dict):
        ...
        def build_item(node_dict: dict, parent_item=None):
            ...
            item = self._make_item(text, None, parent_item)
    ...
    def save_state(self) -> dict:
        """
        Save the current tree configuration into an internal variable and also return it.
        (You can store the returned dict anywhere you like as well.)
        """
        snapshot = self.to_dict()
        self._saved_state = snapshot
        return snapshot

    def has_saved_state(self) -> bool:
        """Return True if save_state() has been called and a snapshot exists."""
        return hasattr(self, "_saved_state") and isinstance(self._saved_state, dict)

    def restore_state(self, state: dict | None = None) -> None:
        """
        Restore the tree from a provided state dict, or from the internally saved snapshot
        if state is None.
        """
        if state is None:
            if not self.has_saved_state():
                raise RuntimeError("No saved state available. Call save_state() first.")
    ...
    def _generate_random_id(self) -> str:
        ...
        return "".join(random.choices(chars, k=self._ID_LENGTH))

    def _generate_unique_id(self) -> str:
        """
        Generate an 8-character unique ID using UUID4.
        Much safer and always available in Python.
        """
    ...
    def _assign_id_o(self, item: QtWidgets.QTreeWidgetItem):
        """Assign unique ID to item if not already assigned."""
        if item.data(0, self._ID_ROLE) is None:
            item.setData(0, self._ID_ROLE, self._generate_unique_id())

    def _assign_id(self, item: QtWidgets.QTreeWidgetItem, uid: str):
        """Assign unique ID to item if not already assigned."""
        if uid is None:
            uid = self._generate_unique_id()

        if item.data(0, self._ID_ROLE) is None:
            item.setData(0, self._ID_ROLE, uid)

    def get_item_id(self, item: QtWidgets.QTreeWidgetItem) -> str | None:
        return item.data(0, self._ID_ROLE)

    def get_item_by_id(self, item_id: str) -> QtWidgets.QTreeWidgetItem | None:
        if not item_id:
            return None

        def walk(item):
            if item.data(0, self._ID_ROLE) == item_id:
                return item
            for i in range(item.childCount()):
...
def setup_input_tree(self, root_label=None):
    ...
    self.cstrat_root = self.ctree.add_parent(self.c_well_tops_folder,"Stratigraphy","00XX00XX", True)
    self.cfault_root = self.ctree.add_parent(self.c_well_tops_folder, "Faults", "00XX00YY",True)
    self.cother_root = self.ctree.add_parent(self.c_well_tops_folder,"Other","00XX00ZZ" , True)
...
## Older version of input and window tree to be discarded
