...
class CheckableTree(QtWidgets.QTreeWidget):
    ...
    def _show_context_menu(self, pos: QtCore.QPoint):
        ...
        if chosen == act_check:
            ...
            new_item = self.add_parent(item, "New Parent", True)
...
#        Blocker = QSignalBlocker(self.model())
 ...
 #       Blocker.unblock()
    ...
    def get_item_by_name(
    ...
    ) -> QtWidgets.QTreeWidgetItem | list[QtWidgets.QTreeWidgetItem] | None:
        """
        Find item(s) by name.
        ...
        parent_item : QTreeWidgetItem or None
            If provided, search only inside this folder.
            If None, search entire tree.

        recursive : bool
            If True, search all descendants.
            If False, search only direct children.

        case_sensitive : bool
            If False, match is case-insensitive.

        partial_match : bool
            If True, match if name is contained in item text.

        first_only : bool
            If True, return first match.
            If False, return list of matches.

        Returns
        -------
        QTreeWidgetItem | list[QTreeWidgetItem] | None
        """
        ...
        def matches(text: str) -> bool:
            ...
            if partial_match:
                return name_cmp in cmp_text
            return cmp_text == name_cmp

        def search(item: QtWidgets.QTreeWidgetItem):
            if matches(item.text(0)):
                results.append(item)
                if first_only:
            ...
            if recursive:
                for i in range(item.childCount()):
                    if search(item.child(i)) and first_only:
                        return True
            return False

        # Determine starting nodes
        if parent_item is not None:
            start_nodes = [parent_item]
        else:
            start_nodes = [self.topLevelItem(i) for i in range(self.topLevelItemCount())]

        for node in start_nodes:
            if search(node) and first_only:
                break
